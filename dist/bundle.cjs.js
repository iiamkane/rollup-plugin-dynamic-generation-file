...
function generatePagesJson(inputDir, outputFile) {
    try {
        var pages_1 = [];
        var files = fs__namespace.readdirSync(inputDir);
        files.forEach(function (file) {
            var fullPath = require$$0__namespace.join(inputDir, file, 'router.ts');
            if (fs__namespace.existsSync(fullPath)) {
                pages_1.push(fullPath);
            }
        });
        // 确保输出目录存在
        var outputDir = require$$0__namespace.dirname(outputFile);
        if (!fs__namespace.existsSync(outputDir)) {
            fs__namespace.mkdirSync(outputDir, { recursive: true });
        }
        // 读取现有的 pages.json 文件内容
        var existingPages = [];
        if (fs__namespace.existsSync(outputFile)) {
            existingPages = JSON.parse(fs__namespace.readFileSync(outputFile, 'utf-8'));
        }
        // 如果内容有变化才进行写入操作
        if (JSON.stringify(existingPages) !== JSON.stringify(pages_1)) {
            fs__namespace.writeFileSync(outputFile, JSON.stringify(pages_1, null, 2));
        }
    }
    catch (error) {
...
function dynamicGenerationFile(options) {
    ...
    return {
        ...
        buildStart: function () {
            ...
            var watcher = watch_1(require$$0__namespace.join(inputDir, '**/router.ts'));
...
module.exports = dynamicGenerationFile;
