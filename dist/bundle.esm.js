...
function generatePagesJson(inputDir, outputFile) {
    try {
        var pages_1 = [];
        var files = fs$4.readdirSync(inputDir);
        files.forEach(function (file) {
            var fullPath = require$$0$1.join(inputDir, file, 'router.ts');
            if (fs$4.existsSync(fullPath)) {
                pages_1.push(fullPath);
            }
        });
        // 确保输出目录存在
        var outputDir = require$$0$1.dirname(outputFile);
        if (!fs$4.existsSync(outputDir)) {
            fs$4.mkdirSync(outputDir, { recursive: true });
        }
        // 读取现有的 pages.json 文件内容
        var existingPages = [];
        if (fs$4.existsSync(outputFile)) {
            existingPages = JSON.parse(fs$4.readFileSync(outputFile, 'utf-8'));
        }
        // 如果内容有变化才进行写入操作
        if (JSON.stringify(existingPages) !== JSON.stringify(pages_1)) {
            fs$4.writeFileSync(outputFile, JSON.stringify(pages_1, null, 2));
        }
    }
    catch (error) {
        console.error("Error dynamic-generation-file: ".concat(error.message));
    }
}
function dynamicGenerationFile(options) {
    var inputDir = options.inputDir, outputFile = options.outputFile;
    return {
        name: 'dynamic-generation-file',
        buildStart: function () {
            generatePagesJson(inputDir, outputFile);
            var watcher = watch_1(require$$0$1.join(inputDir, '**/router.ts'));
            var debouncedGeneratePagesJson = debounce(function () {
                generatePagesJson(inputDir, outputFile);
            }, 300);
...
export { dynamicGenerationFile as default };
